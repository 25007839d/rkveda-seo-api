...
async function getConnectionByProject(projectId){ await db.query(`CREATE TABLE IF NOT EXISTS gbp_connections (id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY, project_id BIGINT UNSIGNED NOT NULL, account_id VARCHAR(255) NULL, location_id VARCHAR(255) NULL, location_name VARCHAR(255) NULL, access_token TEXT NULL, refresh_token TEXT NULL, token_expiry DATETIME NULL, status ENUM('connected','disconnected','error','pending') NOT NULL DEFAULT 'pending', last_synced_at DATETIME NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, UNIQUE KEY uq_gbp_project(project_id), CONSTRAINT fk_gbp_project FOREIGN KEY(project_id) REFERENCES seo_projects(id) ON DELETE CASCADE) ENGINE=InnoDB`); const [rows]=await db.execute('SELECT * FROM gbp_connections WHERE project_id=? LIMIT 1',[projectId]); return rows[0]||null; }
async function disconnectConnection(projectId, userId){
  const c=await getConnection(projectId,userId);
  if(!c) return {disconnected:true,projectId:Number(projectId),existed:false};
  await db.execute(`DELETE FROM gbp_connections WHERE project_id=?`,[projectId]);
...
module.exports={SCOPE,ensureTables,authorizationUrl,exchangeCode,saveConnection,getConnection,getConnectionByProject,listAccounts,listLocations,getLocation,selectLocation,disconnectConnection,performance,reviews,syncLocation};
